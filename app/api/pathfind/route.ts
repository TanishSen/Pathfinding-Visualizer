...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runPathfindingAlgorithm(grid, start, end, algorithm)
...
async function runPathfindingAlgorithm(
  ...
  const manhattanDistance = (a: { row: number; col: number }, b: { row: number; col: number }) => {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          if (grid[row][col] !== "wall") {
            const key = `${row},${col}`
            distances.set(key, row === start.row && col === start.col ? 0 : Number.POSITIVE_INFINITY)
            unvisited.add(key)
          }
        }
      }
      ...
      while (unvisited.size > 0) {
        ...
        if (current.row === end.row && current.col === end.col) {
          ...
          while (pathNode) {
            ...
            const key = `${pathNode.row},${pathNode.col}`
            pathNode = previous.get(key) ? { row: previous.get(key)!.row, col: previous.get(key)!.col } : undefined
          }
        ...
        for (const [dr, dc] of directions) {
          ...
          if (isValid(newRow, newCol) && unvisited.has(neighborKey)) {
            ...
            const currentNeighborDistance = distances.get(neighborKey) || Number.POSITIVE_INFINITY
    ...
    case "astar": {
      ...
      while (openSet.length > 0) {
        ...
        for (const [dr, dc] of directions) {
          ...
          const currentGScore = gScore.get(neighborKey) || Number.POSITIVE_INFINITY

          if (tentativeGScore < currentGScore) {
            gScore.set(neighborKey, tentativeGScore)
            const heuristic = manhattanDistance({ row: newRow, col: newCol }, end)
